...
const ParentPanel = () => {
  ...
  const fetchHomeworks = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/homeworks/student/3', {
  ...
  const fetchAttendance = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get('http://localhost:5000/api/attendance/student/3', { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleFeedbackSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:5000/api/feedbacks', { mesaj: feedback }, {
        headers: { Authorization: `Bearer ${token}` }
      });
